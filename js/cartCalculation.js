document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateCartTotal(total) {
    const subtotalElement = document.querySelector(
      ".cart-total .subtotal:nth-child(2) p:nth-child(2)"
    );
  ...
  function isCartEmpty() {
    ...
    rows.forEach((row) => {
      if (row.querySelector('input[type="number"]')) {
        ...
        if (quantity > 0) {
          cartEmpty = false; // If any item has quantity > 0, cart is not empty
        }
      }
    });
  ...
  document
    .querySelectorAll('.cart-list input[type="number"]')
    .forEach((input) => {
  ...
  const updateCartButton = document.querySelector(
    ".cart-action-button:nth-child(2)"
  );
  if (updateCartButton) {
