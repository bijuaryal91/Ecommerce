...
const firstCardWidth = Math.round(carousel.querySelector(".product").offsetWidth + 15);
const arrowBtns = document.querySelectorAll(".wrapper .indicators");
...
arrowBtns.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    carousel.scrollLeft = Math.round(carousel.scrollLeft + scrollAmount);
  });
});
...
const dragging = (e) => {
  ...
  const moveAmount = startScrollLeft - (e.pageX - startX);
  carousel.scrollLeft = Math.round(moveAmount);
};
...
const autoPlay = () => {
  ...
  timeoutId = setTimeout(() => {
    carousel.scrollLeft = Math.round(carousel.scrollLeft + firstCardWidth);
  }, 2500);
};
