...
if (!empty($code_name) && !empty($dates) && !empty($remaining)) {
    if (is_numeric($remaining) && $remaining >= 0) {
        if (is_numeric($price) && $price >= 0) {
            $sql = "UPDATE redeemcode set code='$code_name',price='$price',expires_at='$formattedDateTime',remaining_Usage='$remaining' WHERE code_Id='$id'";
